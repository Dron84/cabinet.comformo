...
';
    ...
    private function htmlCompanyUnderRegistration(array $userData){
        return $html = "<!DOCTYPE HTML PUBLIC \"-//W3C//DTD HTML 4.0 Transitional//EN\">
            <html lang=\"en\">
            <head>
                <meta charset=\"UTF-8\">
                <meta name=\"viewport\"
            ...
            <table border=\"0\" cellpadding=\"0\" cellspacing=\"0\" style=\"margin:0 auto; padding:0; max-width: 500px; width: 100%; background-color: #ffffff;\">
                <tr>
                    <td>
                        ...
                        <table border=\"0\" cellpadding=\"0\" cellspacing=\"0\" style=\"margin:0; padding:0; width: 100%;
                        background-image: url(http://comformo.com/wp-content/uploads/2019/07/company-under-registration.jpg);
                        background-repeat: no-repeat;
                        background-size: cover;
                        background-position: center;\">
                            <tr>
...
//			echo $sql;
    ...
    {
        $postArray['userid']='30'; //default manager id
...
//		$agent[]= array('index'=>null,'label'=> 'Agents');
        $agent = [];
//		$applicant[]= array('index'=>null,'label'=> 'Admins');
        $admin = [];
...
//		print_r($res);
    ...
    public function getInvoces($userId, $companyId){
        return $this->sqlQuery("SELECT * FROM `invoce` WHERE userId='{$userId}' AND companyId='{$companyId}'");
    }
    ...
    public function addLog(array $postArray)
    {
        $postArray['date'] = date("Y-m-d H:i:s");;
        $data = $this->postToInsert($postArray);
        $sql = "INSERT INTO `log` (" . $data['keys_string'] . ") VALUES (" . $data['values_string'] . ")";
        return $this->sqlQuery($sql);
    }

    public function loadUserLog($userId){
        return $this->sqlQuery("SELECT * FROM `log` WHERE userId='{$userId}'");
    }
...
//	{
//		$sql = "INSERT INTO `users` (email,pass,lastname,firstname,premission_group,applicant)VALUES('$email','" . base64_encode($password) . "','$lastname','$firstname','$premissionGroup','0');";
//		$res = $this->sqlQuery($sql);
//		if ($res ) {
//			echo 'ok';
//		} else {
