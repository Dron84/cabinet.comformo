...
if (isset($_POST['loginin'])) {
    ...
    } else if($_POST['invoce']=='load'){
        unset($_POST['invoce']);
        $userId = explode(',',$_POST['ids'])[0];
        $companyId = explode(',',$_POST['ids'])[1];
        echo json_encode($func->getInvoces($userId,$companyId));
    }
}else if(isset($_POST['log'])){
    if($_POST['log']=='add'){
        unset($_POST['log']);
        echo json_encode($func->addLog($_POST));
    }else if($_POST['log']=='load'){
        unset($_POST['log']);
        echo json_encode($func->loadUserLog($_POST['userId']));
    }
